 ...
 * SMART SYNC SYSTEM V32 — EGRESS OPTIMIZER EDITION
 * ============================================================
 *
 * 🔴 V31 সমস্যা যা ঠিক করা হয়েছে:
 *   - Realtime WebSocket বন্ধ (সবচেয়ে বড় Egress খরচ ছিল)
 *   - Pull interval: 5min → 15min (3x কম request)
 *   - auto-heal.js এর সাথে duplicate fetch বন্ধ
 *   - Page visibility API: tab hidden থাকলে sync বন্ধ
 *   - Compressed payload: students photo base64 strip
 *   - Version-only check: full data pull এর আগে শুধু version চেক
 *
 * ✅ V32 নতুন Features:
 *   - EGRESS GUARD: দৈনিক request count track করে
 *   - SMART PULL: version number match হলে data pull করে না
 *   - TAB VISIBILITY: browser tab minimize/switch হলে sync pause
 *   - SINGLE SOURCE: auto-heal এর cloud fetch বন্ধ করে দেয়
 *   - V31 এর সব partial sync features বজায় আছে
 *
...
(function () {
  ...
  function _patchSaveToStorage() {
    ...
    window.saveToStorage = function (...args) {
      if (typeof origSave === 'function') origSave.apply(this, args);
    };
  ...
  // ─────────────────────────────────────────────────────────
  // AUTO-SAVE MONITOR (localStorage change detect)
  // ─────────────────────────────────────────────────────────
  function _installAutoSaveMonitor() {
    let _lastSnapshot = '';

    setInterval(() => {
      const current = localStorage.getItem('wingsfly_data') || '';
      if (current !== _lastSnapshot && current.length > 10) {
        _lastSnapshot = current;
        // Data বদলেছে — push schedule করো
        if (window.markDirty) window.markDirty();
        schedulePush('Auto-monitor detected change');
      }
    }, 2000);
  }
