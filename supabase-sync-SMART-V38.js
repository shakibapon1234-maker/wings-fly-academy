 ...
 * SMART SYNC SYSTEM — V38.0 "DUPLICATE GUARD + SNAPSHOT REBUILD FIX"
 * ════════════════════════════════════════════════════════════════
 *
...
(function () {
  ...
  async function pullFromCloud(showUI = false, forceFullPull = false) {
    ...
    try {
      ...
      if (_partialOK) {
        ...
        _saveLocal();
        // ✅ V38 FIX: Always rebuild snapshots after pull so next push won't re-push everything
        _rebuildSnapshots();
  ...
  function _getDelta(table, records, keyFn) {
    ...
    // ✅ V38: Track if snapshot was empty (means this browser never pushed before)
  ...
  // ✅ V38: Rebuild full snapshot from current globalData (call after pull or import)
  function _rebuildSnapshots() {
    ...
    try {
      ...
      _saveSnapshot('finance', fSnap);
      _log('📸', `V38 Snapshots rebuilt: ${Object.keys(sSnap).length}S ${Object.keys(fSnap).length}F`);
    ...
    try {
      ...
      if (_partialOK) {
        ...
        if (_dirty.has('students') || _dirty.size === 0) {
          const { changed, deleted, snapshot, snapshotWasEmpty: stuSnapEmpty } = _getDelta('students', gd.students || [], s => s.studentId || s.id || s.name);
          stuTotal = (gd.students || []).length;
          stuPushed = changed.length + deleted.length;
          
          const doStuPush = async () => {
            // ✅ V38 DUPLICATE GUARD: snapshot ছিল না কিন্তু cloud-এ already data থাকলে push skip করো
            if (stuSnapEmpty && changed.length > 0) {
              try {
                const _CFG2 = window.SUPABASE_CONFIG || {};
                const _aid2 = encodeURIComponent(_CFG2.ACADEMY_ID || 'wingsfly_main');
                const _chkRes = await fetch(`${_CFG2.URL}/rest/v1/wf_students?academy_id=eq.${_aid2}&deleted=eq.false&select=id`, {
                  headers: { apikey: _CFG2.KEY, Authorization: 'Bearer ' + _CFG2.KEY, Prefer: 'count=exact', Range: '0-99999' }
                });
                const _cloudCount = parseInt(_chkRes.headers.get('content-range')?.split('/')[1] || '0', 10);
                if (_cloudCount >= stuTotal) {
                  _log('⏭️', `Students SKIPPED — cloud has ${_cloudCount} >= local ${stuTotal}, snapshot rebuild only`);
                  _saveSnapshot('students', snapshot);
                  return;
                }
                _log('📤', `Students: cloud has ${_cloudCount} < local ${stuTotal} — pushing all`);
              } catch(_chkErr) {
                _log('⚠️', 'Students cloud-count check failed, proceeding', _chkErr);
              }
            }
            if (changed.length > 0) {
        ...
        if (_dirty.has('finance') || _dirty.size === 0) {
          const { changed, deleted, snapshot: finSnapshot, snapshotWasEmpty: finSnapEmpty } = _getDelta('finance', gd.finance || [], f => f.id || f.timestamp);
          ...
          const doFinPush = async () => {
            ...
            if (finSnapEmpty && changed.length > 0) {
              try {
                const _CFG3 = window.SUPABASE_CONFIG || {};
                const _aid3 = encodeURIComponent(_CFG3.ACADEMY_ID || 'wingsfly_main');
                const _chkRes3 = await fetch(`${_CFG3.URL}/rest/v1/wf_finance?academy_id=eq.${_aid3}&deleted=eq.false&select=id`, {
                  headers: { apikey: _CFG3.KEY, Authorization: 'Bearer ' + _CFG3.KEY, Prefer: 'count=exact', Range: '0-99999' }
                });
                const _cloudCount3 = parseInt(_chkRes3.headers.get('content-range')?.split('/')[1] || '0', 10);
                if (_cloudCount3 >= finTotal) {
                  _log('⏭️', `Finance SKIPPED — cloud has ${_cloudCount3} >= local ${finTotal}, snapshot rebuild only`);
                  _saveSnapshot('finance', finSnapshot);
                  return;
                }
                _log('📤', `Finance: cloud has ${_cloudCount3} < local ${finTotal} — pushing all`);
              } catch(_chkErr3) {
                _log('⚠️', 'Finance cloud-count check failed, proceeding', _chkErr3);
              }
            }
            if (changed.length > 0) {
              const finRows = changed.map(f => ({ id: `${CFG.ACADEMY_ID}_fin_${f.id || f.timestamp}`, academy_id: CFG.ACADEMY_ID, data: f, deleted: false }));
  ...
  async function _start() {
    ...
    _log('🚀', '══════════════════════════════════════');
    _log('🚀', 'Wings Fly V38.0 — DUPLICATE GUARD + SNAPSHOT REBUILD');
    _log('🚀', '══════════════════════════════════════');
  ...
  async function forcePushOnly(reason) {
    ...
    // ✅ V38 FIX: Clear snapshots so duplicate guard triggers cloud count check
    localStorage.removeItem('wf_push_snapshot_students');
    localStorage.removeItem('wf_push_snapshot_finance');
    _log('🗑️', 'V38: Snapshots cleared before force push — cloud count will be verified');

    const ok = await pushToCloud(reason || 'forcePushOnly');
    if (ok) {
      // ✅ V38 FIX: Rebuild snapshots after successful push (prevents future duplicates)
      _rebuildSnapshots();
